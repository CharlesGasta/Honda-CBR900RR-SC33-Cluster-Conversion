...
constexpr const char *FW_VERSION = "V20.4 SMOOTH RPM";
...
constexpr uint8_t TACH_PULSES_PER_REVOLUTION = 2;
...
struct Settings {
  ...
  // Shift-light moteur chaud.
  uint16_t shiftOnRpm;
  uint16_t shiftFlashRpm;

  // Protection moteur froid.
  uint16_t coldShiftRpm;
  float coldShiftTempC;
...
void loadDefaultSettings() {
  ...
  settings.coldShiftRpm = 8000;
  settings.coldShiftTempC = 60.0f;
...
bool settingsAreValid() {
  ...
  if (settings.coldShiftRpm < 3000 || settings.coldShiftRpm > 14000) return false;
  if (!isfinite(settings.coldShiftTempC) ||
      settings.coldShiftTempC < 20.0f ||
      settings.coldShiftTempC > 100.0f) return false;
...
// RPM reactif utilise par le shift-light.
uint16_t activeRpm = 0;

// RPM filtre uniquement pour l'affichage Wi-Fi.
float displayRpmFiltered = 0.0f;
bool displayRpmInitialized = false;
...
void updateDisplayRpmFilter(uint16_t rpm) {
  if (!displayRpmInitialized) {
    displayRpmFiltered = static_cast<float>(rpm);
    displayRpmInitialized = true;
    return;
  }

  const float error = static_cast<float>(rpm) - displayRpmFiltered;
  const float absError = fabsf(error);

  // Filtre adaptatif : stable au ralenti, plus rapide en acceleration.
  float alpha = 0.18f;
  if (absError >= 150.0f) alpha = 0.30f;
  if (absError >= 500.0f) alpha = 0.48f;
  if (absError >= 1500.0f) alpha = 0.68f;

  displayRpmFiltered += error * alpha;

  if (displayRpmFiltered < 0.0f) displayRpmFiltered = 0.0f;
  if (displayRpmFiltered > 16000.0f) displayRpmFiltered = 16000.0f;
}
...
bool coldShiftActive() {
  return sensorState == SENSOR_OK &&
         isfinite(calculatedTempC) &&
         calculatedTempC < settings.coldShiftTempC;
}
...
void updateShiftLight(uint32_t nowMs) {
  if (!shiftEnabled) {
    ...
    if (coldShiftActive()) {
      // MOTEUR FROID :
      // un seul seuil. Le voyant clignote a partir de coldShiftRpm.
      shiftOnLatched = false;
      shiftAboveOnSinceMs = 0;

      if (rpm >= settings.coldShiftRpm) {
        ...
        if (rpm + SHIFT_HYSTERESIS_RPM < settings.coldShiftRpm) {
          shiftFlashLatched = false;
        }
      }
      ...
      // MOTEUR CHAUD :
      // comportement normal fixe puis clignotant.
  ...
  if (mode != lastLoggedShiftMode) {
    ...
    else if (coldShiftActive()) addLog("Shift-light FROID FLASH");
    else if (mode == 1) addLog("Shift-light CHAUD FIXE");
    else addLog("Shift-light CHAUD FLASH");
  }
}
...
String buildStatusJson() {
  ...
  s += ",";
  s += "\"display\":";
  s += String(static_cast<uint16_t>(displayRpmFiltered + 0.5f));
  s += ",";
  s += "\"periodUs\":";
  ...
  s += ",";
  s += "\"coldRpm\":";
  s += String(settings.coldShiftRpm);
  s += ",";
  s += "\"coldTemp\":";
  s += String(settings.coldShiftTempC, 1);
  s += "},";
...
<section id="settings" class="page">
  ...
  <div class="subpageBody">
    ...
    <div class="panel">
      <h2>Shift-light</h2>
      <div class="two">
        <div><label>À chaud - fixe (tr/min)</label><input id="setShiftOn" type="number" min="1000" max="15000"></div>
        <div><label>À chaud - clignotant (tr/min)</label><input id="setShiftFlash" type="number" min="1000" max="16000"></div>
        <div><label>À froid - limite shift (tr/min)</label><input id="setColdShiftRpm" type="number" min="3000" max="14000"></div>
        <div><label>Passage froid → chaud (°C)</label><input id="setColdShiftTemp" type="number" min="20" max="100" step="0.5"></div>
      </div>
      <div class="note">
        Sous la température réglée, le shift-light clignote dès la limite à froid.
        À partir de cette température, les seuils normaux à chaud prennent le relais.
      </div>
    </div>
    ...
    <div class="panel">
      ...
      <div class="note">
        ...
        shift froid 8000 tr/min sous 60 °C, shift chaud 9000/10000 tr/min,
        NTC 35000 Ω @ 25 °C et 1250 Ω @ 102,5 °C,
...
let rpmTarget=0;
let rpmVisual=0;
let lastNeedleFrame=performance.now();

function animateTachometer(now){
  const dt=Math.min(0.05,Math.max(0.001,(now-lastNeedleFrame)/1000));
  lastNeedleFrame=now;

  const delta=rpmTarget-rpmVisual;
  const absDelta=Math.abs(delta);

  // Inertie adaptative : douce sur petites variations,
  // beaucoup plus rapide lors d'une vraie montee en regime.
  let response=7.0;
  if(absDelta>=120) response=9.5;
  if(absDelta>=500) response=13.0;
  if(absDelta>=1500) response=18.0;

  const blend=1-Math.exp(-response*dt);
  rpmVisual+=delta*blend;

  if(Math.abs(delta)<0.5) rpmVisual=rpmTarget;

  const shown=Math.max(0,Math.round(rpmVisual/10)*10);
  const angle=-135+270*Math.min(1,Math.max(0,rpmVisual)/14000);

  needle.style.transform=`translate(-50%,-92%) rotate(${angle}deg)`;
  rpmDigital.textContent=shown;
...
async function updateStatus(){
  try{
    ...
    // Nouvelle valeur cible. L'animation 60 fps deplace l'aiguille en continu.
    rpmTarget=Number.isFinite(d.rpm.display)?d.rpm.display:d.rpm.value;

    shiftLamp.classList.toggle('on',d.shift.mode==='FIXE'||d.shift.mode==='FLASH');
    if(d.shift.thermalMode==='FROID'){
      shiftText.textContent='FROID';
      shiftThresholds.textContent=d.shift.coldRpm+' tr/min · < '+d.shift.coldTemp.toFixed(0)+' °C';
    }else{
...
--- COMPTE-TOURS ---
RPM contrôle        ${d.rpm.value}
RPM affichage       ${d.rpm.display}
RPM brut            ${d.rpm.raw}
...
Mode thermique       ${d.shift.thermalMode}
Etat                 ${d.shift.mode}
Froid limite         ${d.shift.coldRpm} tr/min
Seuil froid/chaud    ${d.shift.coldTemp.toFixed(1)} °C
Chaud fixe           ${d.shift.on} tr/min
Chaud flash          ${d.shift.flash} tr/min
...
async function loadSettings(){
  ...
  setColdShiftRpm.value=d.shift.coldRpm;
  setColdShiftTemp.value=d.shift.coldTemp;
...
async function saveSettings(){
  const p=new URLSearchParams({
    ...
    coldShiftRpm:setColdShiftRpm.value,coldShiftTempC:setColdShiftTemp.value,
...
void handleSettingsPost() {
  ...
  settings.coldShiftRpm =
    static_cast<uint16_t>(argLong("coldShiftRpm", settings.coldShiftRpm));

  settings.coldShiftTempC =
    argFloat("coldShiftTempC", settings.coldShiftTempC);
...
void handleCalibrateTachIdle() {
  ...
  if (factor < 0.50f || factor > 1.50f) {
    server.send(400, "text/plain; charset=utf-8",
      String("Tarage refuse : RPM brut = ") + String(rawRpm) +
      String(" tr/min, facteur requis = ") + String(factor, 5) +
      String(". Verifier le signal compte-tours."));
    return;
  }
...
void printStatus() {
  ...
  Serial.print("RPM affichage filtre : ");
  Serial.println(static_cast<uint16_t>(displayRpmFiltered + 0.5f));

  Serial.print("Facteur tarage RPM : ");
  Serial.println(settings.tachCalibrationFactor, 5);

  Serial.print("Shift mode thermique : ");
  Serial.println(coldShiftActive() ? "FROID" : "CHAUD");

  Serial.print("Shift froid : ");
  Serial.print(settings.coldShiftRpm);
  Serial.print(" rpm sous ");
  Serial.print(settings.coldShiftTempC, 1);
  Serial.println(" C");

  Serial.print("Shift chaud fixe / flash : ");
...
void printHelp() {
  ...
  Serial.println("  coldshift 3000..14000");
  Serial.println("  coldtemp 20..100");
  Serial.println("  wifi");
...
void processCommand(char *cmd) {
  ...
  if (strncmp(cmd, "coldshift ", 10) == 0) {
    const long value = strtol(cmd + 10, nullptr, 10);

    if (value < 3000 || value > 14000) {
      Serial.println("Erreur : coldshift 3000..14000");
      return;
    }

    settings.coldShiftRpm = static_cast<uint16_t>(value);
    saveSettings();

    Serial.print("Shift froid = ");
    Serial.println(settings.coldShiftRpm);
    return;
  }

  if (strncmp(cmd, "coldtemp ", 9) == 0) {
    const float value = atof(cmd + 9);

    if (value < 20.0f || value > 100.0f) {
      Serial.println("Erreur : coldtemp 20..100");
      return;
    }

    settings.coldShiftTempC = value;
    saveSettings();

    Serial.print("Temperature passage froid/chaud = ");
    Serial.print(settings.coldShiftTempC, 1);
    Serial.println(" C");
    return;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== CBR900RR V20.4 SMOOTH RPM ===");
  Serial.print("Reset : ");
...
void loop() {
  ...
  if (nowMs - lastRpmUpdateMs >= RPM_UPDATE_MS) {
    ...
    updateDisplayRpmFilter(activeRpm);
  }
